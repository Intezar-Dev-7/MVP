...
class AuthServices {
  Future<void> saveUserToBackend({String? token}) async {
    try {
      ...
      print(
        "📍 TOKEN_PROVIDED: ${token != null ? 'YES (${token.substring(0, 20)}...)' : 'NO'}",
      ); //new temp change by aditya

      print(
        "🚀 [BEFORE HTTP.POST] Initiating POST request...",
      ); //new temp change by aditya
      ...
      print(
        "🚀 [AFTER HTTP.POST] Response received! Status: ${response.statusCode}",
      ); //new temp change by aditya
      ...
      if (response.statusCode == 200) {
        ...
        print(
          "❌ BACKEND_ERROR: Status ${response.statusCode}",
        ); //new temp change by aditya
        print(
